<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:pmd="http://pmd.sourceforge.net/report/2.0.0">
	...
	<xsl:template name="timestamp">
		...
		<xsl:value-of select="substring-before(//pmd:pmd/@timestamp, 'T')" />
		-
		<xsl:value-of select="substring-before(substring-after(//pmd:pmd/@timestamp, 'T'), '.')" />
	</xsl:template>
	...
	<xsl:template match="pmd:pmd" mode="rulelist">
		...
		<table class="log" border="0" cellpadding="5" cellspacing="2" width="100%">
			...
			<xsl:for-each select="pmd:file/pmd:violation[@rule and generate-id(.) = generate-id(key('violations', @rule))]">
				...
				<xsl:variable name="violationCount" select="count(../../pmd:file/pmd:violation[@rule=$currentRule])" />
		...
		<table class="log" border="0" cellpadding="5" cellspacing="2" width="100%">
			...
			<xsl:for-each select="pmd:file">
				...
				<tr>
					...
					<td>
						<xsl:value-of select="count(pmd:violation[@priority = 5])" />
					</td>
					<td>
						<xsl:value-of select="count(pmd:violation[@priority = 4])" />
					</td>
					<td>
						<xsl:value-of select="count(pmd:violation[@priority = 3])" />
					</td>
					<td>
						<xsl:value-of select="count(pmd:violation[@priority = 2])" />
					</td>
					<td>
						<xsl:value-of select="count(pmd:violation[@priority = 1])" />
					</td>
				</tr>
	...
	<xsl:template match="pmd:pmd" mode="summary">
		...
		<table class="cicd">
			...
			<tr>
				<td>
					START_TOTAL_FILES<xsl:value-of select="count(//pmd:file)" />END_TOTAL_FILES
				</td>
				<td>
		...
		<table class="log" border="0" cellpadding="5" cellspacing="2" width="100%">
			...
			<tr>
				<xsl:call-template name="alternated-row" />
				<td>
					<xsl:value-of select="count(//pmd:file)" />
				</td>
				<td>
					<xsl:value-of select="count(//pmd:violation)" />
				</td>
				<td>
					<xsl:value-of select="count(//pmd:violation[@priority = 1])" />
				</td>
